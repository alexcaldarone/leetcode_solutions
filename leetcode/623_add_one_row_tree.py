...
class Solution:
    def addOneRow(self, root: Optional[TreeNode], val: int, depth: int) -> Optional[TreeNode]:
        ...
        def insert(val, node, node_depth, n):
            if not node:
                return

            if node_depth == n-1:
                print(node_depth, n, node)
                tmp_left = node.left
                tmp_right = node.right
                new_l, new_r = TreeNode(val), TreeNode(val)
                node.left = new_l
                node.right = new_r
                new_l.left = tmp_left
                new_r.right = tmp_right
            else:
                insert(val, node.left, node_depth+1, n)
                insert(val, node.right, node_depth+1, n)
