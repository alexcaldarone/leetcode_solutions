...
The master server will process all newly arrived messages instantly and send a reply to the originating server via the reversed path 
...
If it has not, it will resend the message periodically. The data server i will resend the message every patience[i] second(s), i.e., 
...
class Solution:
    def networkBecomesIdle(self, edges: List[List[int]], patience: List[int]) -> int:
        ...
        for source, dest in edges:
            neigs[source].append(dest)
            neigs[dest].append(source)
        print(neigs)

        times = [float("inf")] * len(patience)
        ...
        while q:
            ...
            for neig in neigs[node]:
                if curr_dist + 1 < times[neig]:
                    times[neig] = curr_dist + 1
                    q.append((times[neig], neig))
        
        res = float("-inf")
        for i in range(1, len(patience)):
            if patience[i] < 2*times[i]:
                rem = 2*times[i] % patience[i]
                last_message = 2*times[i]-rem if rem > 0 else 2*times[i]-patience[i]
                res = max(res, last_message + 2*times[i])
            else:
                res = max(res, 2*times[i])
