...
class Solution:
    def canIWin(self, maxChoosableInteger: int, desiredTotal: int) -> bool:
        # structure solution recursively
        def can_win(available: int, remaining: int):
            nonlocal dp
            ...
            for x in range(1, maxChoosableInteger+1):
                if x >= remaining and not (available & (1 << x)):
                    dp[available] = True
                    return True
                
            for x in range(1, maxChoosableInteger+1):
                if not (available & (1 << x)):
                    if not can_win(available | (1 << x), remaining - x):
                        dp[available] = True
                        return True
